...
int IsBst2(tr* root, int min, int max)
{
	if(!root)
		return 1;
	return(min < root->info && root->info < max && IsBst2(root->left, min, root->info) && IsBst2(root->right, root->info, max));
}
...
int main()
{
	...
	display(start);
	if(IsBst2(start, INT_MIN, INT_MAX))
		printf("It's a BST\n");
	else
		printf("Not a BST\n");
	delete_tree(start);
